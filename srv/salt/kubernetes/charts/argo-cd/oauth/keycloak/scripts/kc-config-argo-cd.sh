...
# ------------------- | -----------------------
# argo-cd-admins    | cluster-admin
# argo-cd-users     | view
##################################################
function create-groups {
    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="argo-cd-admins")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "argo-cd admins group already exists"
    fi

    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="argo-cd-users")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "argo-cd users group already exists"
    fi
}
...
function create-client {
  ...
  then
    ...
    echo "argo-cd client already exists"
  fi
  ...
  if ! http GET \
    ...
    "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="audience-argo-cd")'
  then
    http --pretty=none POST \
      "${URL}/auth/admin/realms/${REALM}/client-scopes/${CSID}/protocol-mappers/models" \
      'Content-Type: application/json' \
  ...
  CID=$(http GET \
    "${URL}/auth/admin/realms/${REALM}/clients" \
    "Authorization: Bearer ${TOKEN}" | jq -M -e -r '.[] | select(.clientId=="argo-cd") | .id'
  )
