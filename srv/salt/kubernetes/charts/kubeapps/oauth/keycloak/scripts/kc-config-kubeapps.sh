...
# ------------------- | -----------------------
# kubeapps-admins    | cluster-admin
# kubeapps-users     | view
##################################################
function create-groups {
    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="kubeapps-admins")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "kubeapps admins group already exists"
    fi

    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="kubeapps-users")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "kubeapps users group already exists"
    fi
}
...
function create-client {
  ...
  then
    ...
    echo "kubeapps client already exists"
  fi
  ...
  if ! http GET \
    ...
    "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="audience-kubeapps")'
  then
    http --pretty=none POST \
      "${URL}/auth/admin/realms/${REALM}/client-scopes/${CSID}/protocol-mappers/models" \
      'Content-Type: application/json' \
    ...
    echo "kubeapps protocolmapper already exists"
  fi

  CID=$(http GET \
    "${URL}/auth/admin/realms/${REALM}/clients" \
    "Authorization: Bearer ${TOKEN}" | jq -M -e -r '.[] | select(.clientId=="kubeapps") | .id'
  )
