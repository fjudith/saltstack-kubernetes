...
# ------------------- | -----------------------
# openfaas-admins    | cluster-admin
# openfaas-users     | view
##################################################
function create-groups {
    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="openfaas-admins")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "openfaas admins group already exists"
    fi

    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="openfaas-users")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "openfaas users group already exists"
    fi
}
...
function create-client {
  ...
  then
    ...
    echo "openfaas client already exists"
  fi
  ...
  if ! http GET \
    ...
    "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="audience-openfaas")'
  then
    http --pretty=none POST \
      "${URL}/auth/admin/realms/${REALM}/client-scopes/${CSID}/protocol-mappers/models" \
      'Content-Type: application/json' \
    ...
    echo "openfaas protocolmapper already exists"
  fi

  CID=$(http GET \
    "${URL}/auth/admin/realms/${REALM}/clients" \
    "Authorization: Bearer ${TOKEN}" | jq -M -e -r '.[] | select(.clientId=="openfaas") | .id'
  )
