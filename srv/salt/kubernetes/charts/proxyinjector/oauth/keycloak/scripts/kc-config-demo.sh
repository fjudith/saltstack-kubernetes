...
# ------------------- | -----------------------
# demo-admins    | cluster-admin
# demo-users     | view
##################################################
function create-groups {
    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="demo-admins")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "demo admins group already exists"
    fi

    if ! http GET \
      "${URL}/auth/admin/realms/${REALM}/groups" \
      "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="demo-users")'
    then
      http --pretty=none POST \
        "${URL}/auth/admin/realms/${REALM}/groups" \
        'Content-Type: application/json' \
      ...
      echo "demo users group already exists"
    fi
}
...
#    associated to "demo-deck" Service.
# 2. Insert the audience protocol mapper associated 
#    to the "demo" client in the "allowed-services"
#    client scopes.
...
# 6. Update the ingress resource "kubernetes-demo"
#    to point to the Keycloak Gatekeeper proxy.
##################################################
function create-client {
   if ! http GET \
    "${URL}/auth/admin/realms/${REALM}/clients" \
    "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.clientId=="demo")'
  then
    http --pretty=none POST \
      "${URL}/auth/admin/realms/${REALM}/clients" \
      'Content-Type: application/json' \
    ...
    echo "demo client already exists"
  fi
  ...
  if ! http GET \
    ...
    "Authorization: Bearer ${TOKEN}" | jq -M -e '.[] | select(.name=="audience-demo")'
  then
    http --pretty=none POST \
      "${URL}/auth/admin/realms/${REALM}/client-scopes/${CSID}/protocol-mappers/models" \
      'Content-Type: application/json' \
    ...
    echo "demo protocolmapper already exists"
  fi

  CID=$(http GET \
    "${URL}/auth/admin/realms/${REALM}/clients" \
    "Authorization: Bearer ${TOKEN}" | jq -M -e -r '.[] | select(.clientId=="demo") | .id'
  )
