...
locals {
  # the last address is reserved for the gateway
  gateway_ip = "${cidrhost(var.zerotier_cidr, 254)}" # eg 172.17.4.254
}
...
resource "zerotier_network" "kubernetes" {
  ...
  assignment_pool {
    first = "${cidrhost(var.zerotier_cidr,  1)}" # eg 172.17.4.1
    last  = "${cidrhost(var.zerotier_cidr, -5)}" # eg 172.17.4.250
  }
...
resource "scaleway_server" "etcd" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/install-zerotier.sh ${var.zerotier_api_key} ${zerotier_network.kubernetes.id} ${cidrhost(var.zerotier_cidr, var.etcdIP + count.index)}",
    ]
  }
...
resource "scaleway_server" "master" {
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/install-zerotier.sh",
      "/tmp/install-zerotier.sh ${var.zerotier_api_key} ${zerotier_network.kubernetes.id} ${cidrhost(var.zerotier_cidr, var.masterIP + count.index)}",
    ]
  }
...
resource "scaleway_server" "node" {
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/install-zerotier.sh",
      "/tmp/install-zerotier.sh ${var.zerotier_api_key} ${zerotier_network.kubernetes.id} ${cidrhost(var.zerotier_cidr, var.nodeIP + count.index)}",
    ]
  }
